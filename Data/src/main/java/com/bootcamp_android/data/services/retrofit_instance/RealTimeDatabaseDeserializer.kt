...
class RealTimeDatabaseDeserializer : JsonDeserializer<ReservationListResponse> {
    ...
    override fun deserialize(
        ...
        var reservationListResponse = ReservationListResponse(mutableListOf<Reservation>())
            ...
            for(key in keys) {
                try {
                    ...
                    val reservationResponse = Reservation(
                        key,
                        asJsonObject.get(AUTHORIZATION_CODE).asString,
                        asJsonObject.get(START_DATE).asLong,
                        asJsonObject.get(
                            END_DATE
                        ).asLong,
                        asJsonObject.get(PARKING_LOT).asInt
                    )
